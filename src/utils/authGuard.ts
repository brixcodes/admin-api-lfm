...
export function requireAuth(to: any, from: any, next: any) {
  // Si l'utilisateur est déjà connecté et essaie d'accéder à login/register, rediriger vers le dashboard
  if (isAuthenticated() && (to.path === '/login' || to.path === '/register')) {
    next('/dashboard')
    return
  }

  // Dans tous les autres cas, autoriser l'accès sans vérification
  next()
}

// Ces fonctions peuvent être supprimées si elles ne sont plus utilisées
export function getRedirectAfterLogin(): string {
  return '/dashboard'
}

export async function validateTokenAndRedirect(router: any) {
  // Ne rien faire car on ne valide plus le token
}
